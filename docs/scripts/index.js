...
var prismjs = Object.freeze({
	default: prism,
	__moduleExports: prism
});
...
function load(item) {
    return new Promise(function (resolve, reject) {
        ...
        var containerOriginContent = item.rewrite ? '' : container.innerHTML;
        var interval = 16;
        var startLoad = function startLoad() {
            setTimeout(function () {
                ...
                if (num <= sum) {
                    ...
                    // console.log(code)
                    var nextInterval = getInterval(str, interval);
...
var AnimateResume = function () {
    function AnimateResume(options) {
        classCallCheck(this, AnimateResume);

        this.options = options;
    }

    createClass(AnimateResume, [{
        key: 'load',
        value: function load$$1(contents) {
            var _this = this;

            contents.length && load(contents[0]).then(function () {
                return _this.load(contents.slice(1));
            });
        }
    }, {
        key: 'animate',
...
var ar = new AnimateResume({ content: [load1, load2] });
ar.animate();
